...
if( is_page() ):
	...
	kriya_show_sidebar( 'page', $page_id, 'right');
elseif( is_single() ):
	if( is_singular('post') ) {
		$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
		kriya_show_sidebar('post',$id, 'right');
	} elseif( is_singular('dt_portfolios') ) {
		$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
		kriya_show_sidebar('dt_portfolios',$id, 'right');
	} elseif( is_singular('product') ) {
		$enable = kriya_option('woo','show-shop-standard-right-sidebar-for-product-layout');
		if( $enable ):
...
elseif( class_exists('woocommerce') && is_product_category() ):
	...
	$enable = kriya_option('woo','show-shop-standard-right-sidebar-for-product-category-layout');
	if( $enable ):
		if( is_active_sidebar('shop-everywhere-sidebar') ):
			dynamic_sidebar('shop-everywhere-sidebar');
		endif;
...
elseif( is_tax() ):
	...
	if( $tax == 'dt_portfolios_categories' ) {
		...
		$enable =  kriya_option('pageoptions','show-standard-right-sidebar-for-portfolio-archives');
	...
	if( $tax == 'dt_yoga_days' || $tax == 'dt_yoga_classes' || $tax == 'dt_yoga_stages' || $tax == 'dt_yoga_period' ) {
		...
		$enable = kriya_option('pageoptions','show-standard-right-sidebar-for-course-archives');
...
elseif( is_archive() || is_search() ):
	...
	$enable = kriya_option('pageoptions','show-standard-right-sidebar-for-post-archives');
	if($enable):
		if( is_active_sidebar('standard-sidebar') ):
			dynamic_sidebar('standard-sidebar');
		endif;
